...
export function getBuiltInAngelCards() {
  return [
...
export function getBuiltInMudras() {
  return [
    {
      name: "Gyan Mudra",
...
export async function seedDatabase() {
  ...
  const builtInMudraNames = new Set(builtInMudras.map(m => m.name));
  const builtInCardNames = new Set(builtInCards.map(c => c.name));

  const existingMudras = await storage.getMudras();
  const existingMudraNames = new Set(existingMudras.map(m => m.name));

  for (const m of existingMudras) {
    if (!builtInMudraNames.has(m.name)) {
      await storage.deleteMudra(m.id);
    }
  }
  for (const m of builtInMudras) {
    if (!existingMudraNames.has(m.name)) {
